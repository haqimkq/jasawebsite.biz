<x-app-layout title="Edit Label Pelanggan">
    <div class=" my-10 flex justify-center sm:p-0 p-3">
        <div
            ...
            <div class="px-3 pb-3 flex justify-center gap-3">
                <form class="w-full" method="POST" action="{{ route('pelanggans.storeLabel', $pelanggan->id) }}">
                    @csrf
                    <ul
                            ...
                            <li class="w-full border-b border-gray-200 rounded-t-lg dark:border-gray-600">
                                <div class="flex items-center pl-3">
                                    <input id="vue-checkbox-{{ $labels->id }}" type="checkbox" name="labels[]"
                                        value="{{ $labels->id }}"
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500"@if ($pelanggan->label->contains($labels->id)) checked @endif>
                    ...
                    <div class="flex justify-end mt-3">
                        <div class="px-3 py-2 bg-white text-gray-800 rounded-lg font-extrabold text-sm tracking-widest">
                ...
                <div class="">
                    @foreach ($label as $labels)
                        <form action="{{ route('label.destroy', $labels->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <div>
